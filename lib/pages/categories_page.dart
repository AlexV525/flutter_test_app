...
class _CategoriesPageState extends State<CategoriesPage> {
  final _menuItemsResponse = <Map<String, dynamic>>[
    {"id": "1216641992120414210", "pid": "1215566421248524290", "name": "商品库查询"},
    {"id": "1216641327088349186", "pid": "1215566421248524290", "name": "公告"},
    {"id": "1215566686601166850", "pid": "1215566245175836673", "name": "外场操作"},
    {"id": "1217283928581812225", "pid": "1215566491717025793", "name": "财务统计分析"},
    {"id": "1216642256281874433", "pid": "1215566491717025793", "name": "账单信息查询"},
    {"id": "1216642125218263042", "pid": "1215566421248524290", "name": "在线咨询"},
    {"id": "1216641791364247553", "pid": "1215566421248524290", "name": "报关信息查询"},
    {"id": "1215566421248524290", "pid": "1215565952358891522", "name": "委托信息查询"},
    {"id": "1216642382228434945", "pid": "1215566245175836673", "name": "运输操作"},
    {"id": "1217283696011849729", "pid": "1215566491717025793", "name": "发票信息查询"},
    {"id": "1215566491717025793", "pid": "1215565952358891522", "name": "财务数据查询"},
    {"id": "1215566245175836673", "pid": "1215565952358891522", "name": "外场工作平台"},
  ...
  void initState() {
    ...
    _menuItemsResponse.where((item) => item['pid'] == topList.id).forEach((_item) {
    ...
    _menuItemsResponse.forEach((_item) {
      final item = LoginDataUserMenuList.fromJson(_item);
      final key = menuList.keys.firstWhere((list) => list.id == item.pid, orElse: () => null);
    ...
    return GestureDetector(
      ...
      child: SizedBox.fromSize(
        ...
        child: Column(
          ...
          children: <Widget>[
            Image.asset("assets/icons/home/${_menuItemIcons[item.name]}-home.png"),
            Text("${item.name}"),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) => sectionWidget(menuList.keys.elementAt(index)),
      ),
    );
  }
}
...
class LoginDataUserMenuList {
  ...
  bool operator ==(Object other) =>
      ...
      other is LoginDataUserMenuList && runtimeType == other.runtimeType && id == other.id;
